...
_d('CF_DB_TBL_PREFIX',  'cfm_');                                    // Database table prefix
_d('CF_VERSION',        '0.0.1');                                   // System version
_d('CF_BUILD',          'DEV');                                     // Build type
_d('CF_INSTANCE_NAME',  'CF Manager 2 v'.CF_VERSION);               // Instance name
...
_d('CF_LOG_FILENAME',   'app.log');                                 // Log filename
_d('CF_LOG_LEVEL',      Monolog\Logger::DEBUG);                     // Minimum log level
_d('CF_AUTH_HTTP',      false);                                     // Should we use HTTP auth?
_d('CF_AUTH_HTTP_REALM',CF_INSTANCE_NAME);                          // HTTP Authentication realm
...
$settings = [
    'settings' => [
        ...
        'logger' => [
            'name' => 'cfm',
            'path' => CF_DIR_LOGS . CF_LOG_FILENAME,
        ],
    ],
];